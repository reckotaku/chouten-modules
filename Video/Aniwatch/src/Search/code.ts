...
async function logic(payload: BasePayload) {
    ...
    const elements = document.querySelectorAll<HTMLElement>(".film_list-wrap .film-detail > h3 > a");
    const images = document.querySelectorAll(".film_list-wrap .film-poster > img");
    const subDub = document.querySelectorAll<HTMLElement>(".film_list-wrap > div > .film-poster > div.tick.ltr");
    const epCounts = document.querySelectorAll<HTMLElement>(".film_list-wrap > div > .film-poster > .tick.ltr > div");
