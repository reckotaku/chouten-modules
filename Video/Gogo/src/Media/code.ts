...
async function logic(payload: BasePayload) {
    ...
    const encyptedParams = generateEncryptedAjaxParams(embedHTML.split("data-value")[1].split('"')[1], videoURL.searchParams.get("id") ?? "", keys);

    const encryptedData = JSON.parse(
        await sendRequest(`${videoURL.protocol}//${videoURL.hostname}/encrypt-ajax.php?${encyptedParams}`, {
